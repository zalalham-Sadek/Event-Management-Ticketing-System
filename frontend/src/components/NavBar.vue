<template>
  <nav class="bg-[var(--color-bg)] shadow-md sticky top-0 z-50 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">

        <!-- Logo + Menu for desktop -->
        <div class="flex items-center space-x-8">
          <router-link to="/" class="text-2xl font-bold text-[var(--color-primary)]">🎟️ EventSys</router-link>

          <!-- Menu -->
          <div
            :class="[
              'flex flex-col md:flex-row md:space-x-6 font-medium items-center absolute md:static left-0 w-full md:w-auto bg-[var(--color-bg)] md:bg-transparent shadow-md md:shadow-none transition-all duration-300 ease-in-out',
              isOpen ? 'top-16 opacity-100' : 'top-[-400px] opacity-0 md:opacity-100'
            ]"
          >
            <NavItem to="/" label="home" class="mt-2 md:mt-0" />
            <NavItem to="/" label="events" class="mt-2 md:mt-0" />
            <NavItem to="/" label="organizers" class="mt-2 md:mt-0" />
          </div>
        </div>
        ...
        <div class="flex items-center space-x-4">
          <!-- Login Icon -->
          <router-link to="/login" class="text-[var(--color-text)] hover:text-[var(--color-primary)]">
          ...
          <!-- Mobile Menu Toggle -->
          <div class="md:hidden">
            <button @click="isOpen = !isOpen" class="text-[var(--color-text)] hover:text-[var(--color-primary)] focus:outline-none">
